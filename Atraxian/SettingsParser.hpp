...
public:
	...
	bool get(const std::string& key, sf::Vector2f &value) const;
...
inline bool SettingsParser::get(const std::string& key, sf::Vector2f &value) const {
	auto it = m_data.find(key);
	if (it != m_data.end()) {
...
//		value.clear();
		...
		while (getline(parser, output, ',')) {
			things.push_back(std::stoi(output));
		}

		value = { things[0], things[1] };
